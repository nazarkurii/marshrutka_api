...
import (
	...
	"maryan_api/pkg/credentianlsutils"
...
type Ticket struct {
	...
	Seats           []entity.TicketSeat  `json:"seats"`
	PhoneNumber     string               `gorm:"type:varchar(15);not null"                                                  json:"phoneNumber"`
	Email           string               `gorm:"type:varchar(255);not null"                          json:"email"`
	Passengers      []entity.Passenger   `gorm:"foreignKey:TicketID"       json:"passengers"`
	PickUpAdressID  uuid.UUID            `gorm:"type:binary(16);not null"           json:"-"`
	PickUpAdress    entity.Address       `gorm:"foreignKey:PickUpAdressID"    json:"pickUpAddress"`
	DropOffAdressID uuid.UUID            `gorm:"type:binary(16);not null"           json:"-"`
	DropOffAdress   entity.Address       `gorm:"foreignKey:DropOffAdressID"   json:"dropOffAddress"`
	CreatedAt       time.Time            `gorm:"not null"                     json:"createdAt"`
	CompletedAt     sql.NullTime         `                                    json:"completedAt"`
	Payment         entity.TicketPayment `gorm:"foreignKey:TicketID"    `
...
type NewTicketJSON struct {
	...
	Passengers    []entity.NewPassenger `json:"passengers"`
	DropOffAdress entity.NewAddress     `json:"dropOffAdress"`
	PickUpAdress  entity.NewAddress     `json:"pickUpAdress"`
...
func (t NewTicketJSON) ParseContaanctInfo() (email string, phoneNumber string, err error) {
	...
	phoneNumber, err = credentianlsutils.FomratPhoneNumber(t.PhoneNumber)
	if err != nil {
